...
def random_horizontal_flip(
  ...
  with tf.name_scope('RandomHorizontalFlip', values=[image, boxes]):
    ...
    # flip image
    image = tf.cond(
        do_a_flip_random, lambda: _hflip_image(image), lambda: image)
    result.append(image)
    ...
    if masks is not None:
      masks = tf.cond(
          do_a_flip_random, lambda: _hflip_masks(masks), lambda: masks)
      result.append(masks)
...
def random_vertical_flip(
    ...
    keypoint_flip_permutation=None,
    seed=None):
  """Randomly decides whether to vertically mirror the image and detections or
  not.
  ...
  with tf.name_scope('RandomVerticalFlip', values=[image, boxes]):
    ...
    # flip image
    image = tf.cond(
        do_a_flip_random, lambda: _vflip_image(image), lambda: image)
    result.append(image)

    # flip boxes
    if boxes is not None:
      boxes = tf.cond(
          do_a_flip_random, lambda: vflip_boxes(boxes), lambda: boxes)
      result.append(boxes)

    # flip masks
    if masks is not None:
      masks = tf.cond(
          do_a_flip_random, lambda: _vflip_masks(masks), lambda: masks)
      result.append(masks)
...
def get_default_func_arg_map(include_instance_masks=False,
  ...
  prep_func_arg_map = {
      ...
      vflip_boxes: (fields.InputDataFields.groundtruth_boxes,),
